...
Driver boards >=v1.2 are 6 layer PCBs measuring 40×40mm². Note the different position of the MCUs, which makes this version easily distinguishable from [driver-v2.x](#driver-v2). The most recent production files are archived in `production_v1/driver-v1p7.zip` with a changelog available in the `production_v1/README.mdown` file. All components are SMD with the smallest component measuring 0402 (imperial) or 1005 (metric). Therefore factory assembly is recommended.
...
The files starting with version v1.2 are in its own dedicated repository that is linked in the bottom left of the  website. The driver board was designed in-house at Janelia's [jET](https://www.janelia.org/support-team/janelia-experimental-technology) group. The [OrCAD](https://www.orcad.com/) EDA source file is provided for reference, although the latest production files might not always correspond with the design file version.
...
![driver version 0.2, 0.3, or 0.4 with LEDs](assets/driver_v0p2_back_photo.jpg){:.ifr .pop}
